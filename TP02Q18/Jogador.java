class Jogador{
    ...
    public static void quicksort(int esq, int dir , Jogador[] array){
        ...
        while (i <= j) {
            while (array[i].getEstadoNascimento().compareTo(pivo.getEstadoNascimento()) < 0) i++;
            while (array[j].getEstadoNascimento().compareTo(pivo.getEstadoNascimento()) > 0) j--;
            if (i <= j) {
                swap(i, j , array);
                i++;
                j--;
...
public static void main(String[] args){
    ...
    quicksort(0, jogador.length - 1 , jogador);

        for (int i = (numEntrada - 1); i > 0; i--) {
            for (int j = 0; j < i; j++) {
                if (jogador[j].getNome().compareTo(jogador[j + 1].getNome()) > 0 && jogador[j].getEstadoNascimento().compareTo(jogador[j + 1].getEstadoNascimento()) == 0) {
